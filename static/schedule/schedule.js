(() => {
  ...
  function renderCalendar() {
    ...
    state.event.dates.forEach((date) => { const header = document.createElement("div"), parsed = new Date(`${date}T12:00:00`); header.className = "day-label"; header.innerHTML = `<span>${new Intl.DateTimeFormat("en-US", { weekday:"short" }).format(parsed)}</span><strong>${shortDateFormatter.format(parsed)}</strong>`; calendar.appendChild(header); });
    const selected = currentSlots();
    slots.forEach((minute, index) => {
      const label = document.createElement("div"); label.className = "time-label";
      if (index % Math.max(1, 60 / state.event.slot_minutes) === 0) label.textContent = new Intl.DateTimeFormat("en-US", { hour:"numeric", minute:"2-digit" }).format(new Date(`2000-01-01T${fromMinutes(minute)}:00`));
      ...
      state.event.dates.forEach((date) => {
        ...
        if (state.view === "group") { const names = state.responses.filter((response) => response.availability.includes(key)).map((response) => response.display_name); const ratio = count ? names.length / count : 0; cell.classList.add("group"); cell.disabled = true; cell.style.backgroundColor = names.length ? `rgba(42, 73, 151, ${0.16 + ratio * 0.7})` : ""; if (names.length) { cell.title = names.join(", "); cell.setAttribute("aria-label", `${formatDate(date)} ${fromMinutes(minute)}: ${names.join(", ")}`); } }
  ...
  function syncView() {
    ...
    $("mode-mine").classList.toggle("active", mine); $("mode-group").classList.toggle("active", !mine); $("mode-mine").setAttribute("aria-selected", mine); $("mode-group").setAttribute("aria-selected", !mine); $("save-area").classList.toggle("is-hidden", !mine); $("group-summary").classList.toggle("is-hidden", mine);
  ...
  function renderSchedule() { $("event-name").textContent = state.event.title; $("event-meta").textContent = `${formatDate(state.event.dates[0])}${state.event.dates.length > 1 ? ` — ${formatDate(state.event.dates.at(-1))}` : ""} · ${formatClock(state.event.start_time)}–${formatClock(state.event.end_time)} · ${state.event.timezone}`; $("participant-name").value = state.name; syncView(); renderPeople(); renderCalendar(); $("save-availability").disabled = true; }
  ...
  async function saveAvailability() { if (!state.name) return setIdentityStatus("Enter your name first.", true); try { if (online) await saveOnline(); else saveDemo(); state.responses = online ? (await loadOnline(state.event.slug)).responses : state.responses; localStorage.setItem(`moilsigan-name-${state.event.id}`, state.name); setSaveStatus("Saved."); renderSchedule(); } catch { setSaveStatus("Could not save. Check your connection and try again.", true); } }
  function beginName() { const name = $("participant-name").value.trim(); if (!name) { setIdentityStatus("Enter your name first.", true); $("participant-name").focus(); return; } state.name = name; state.ownSlots = new Set(responseForName(name)?.availability || []); localStorage.setItem(`moilsigan-name-${state.event.id}`, name); setIdentityStatus(`Now marking availability as ${name}.`); setSaveStatus(""); renderSchedule(); }
