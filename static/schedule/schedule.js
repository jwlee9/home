    ...
    slots.forEach((minute, index) => {
      ...
      state.event.dates.forEach((date) => {
        ...
        if (state.view === "group") {
          ...
          cell.classList.add("group"); cell.classList.toggle("unanimous", unanimous); cell.style.backgroundColor = names.length ? `rgba(${highlightRgb()}, ${0.16 + ratio * 0.7})` : ""; cell.title = unanimous ? `${everyoneAvailable()}: ${names.join(", ")}` : names.length ? names.join(", ") : tr("noOne");
